...
from tables.portfolio_optimization_dataset import PortfolioOptimizationFullDataset
...
def populate_portfolio_optimization_dataset():
    ...
    stock_price_data = db.Table("stock_price_dataset", metadata, autoload_with=engine)
    
    unique_dates_query = db.select(stock_price_data.columns.date.distinct()).order_by(db.desc(stock_price_data.columns.date))
    ...
    for date_entry in unique_date_result_set:
        ...
        sub_query = db.select(stock_price_data.columns.asset.distinct().label("asset"),db.func.count(stock_price_data.columns.asset).label("asset_count")).where(stock_price_data.columns.date<date).group_by(stock_price_data.columns.asset)
        
        main_query = db.select(sub_query.subquery().columns.asset.distinct()).where(sub_query.subquery().columns.asset_count>=180)
        ...
        portfolio_optimization_dataset_entry = PortfolioOptimizationFullDataset(**portfolio_optimization_dict)
